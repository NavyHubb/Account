...
class AccountServiceTest {
    ...
    void createAccount_Success() {
        ...
        ArgumentCaptor<Account> captor = ArgumentCaptor.forClass(Account.class);  // 실제로 저장될 객체가 담길 변수. given에서 한 건 가짜
        ...
        assertEquals(13, accountDto.getUserId());
        assertEquals("1000000013", captor.getValue().getAccountNumber());
    }

    @Test
    @DisplayName("첫 번째 계좌 생성")
    void createFirst_Success() {
        //given
        AccountUser user = AccountUser.builder()
                .id(11L)
                .name("FourB")
        ...
        //then
        verify(accountRepository, times(1)).save(captor.capture());
        assertEquals(11, accountDto.getUserId());
    ...
    void deleteAccount_balanceNotEmpty() {
        ...
        given(accountRepository.findByAccountNumber(anyString()))
                .willReturn(Optional.of(
                        Account.builder()
                                .accountUser(Green)  // 위에서 찾은 유저는 Green인데 계좌 소유주는 Red
                                .accountNumber("1000000012")
                                .balance(777L)
                                .build()));
